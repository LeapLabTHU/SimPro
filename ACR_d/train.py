...
def compute_py(train_loader, args):
    ...
    # logger.info(label_freq)
    # logger.info(len(label_freq))
...
def main():
    ...
    parser.add_argument("--tau", default=2, type=float, help="useless")
    ...
    file_handler = logging.FileHandler(os.path.join(args.out, f"log.txt"), mode="a")
    ...
    if args.dataset == "cifar10":
        ...
        args.dataset_name = "smallimagenet_1k"
        args.arch = "resnet"

    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()

    labeled_dataset, unlabeled_dataset, test_dataset, cls_num_list_l, cls_num_list_u = (
...
def train(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        semi_acc = AverageMeter()

        if epoch > args.est_epoch:
            count_KL = count_KL / args.eval_step
            KL_softmax = (torch.exp(count_KL[0])) / (
                torch.exp(count_KL[0]) + torch.exp(count_KL[1]) + torch.exp(count_KL[2])
            )
        ...
        for batch_idx in tqdm(range(args.eval_step), total=args.eval_step):
            ...
            inputs = interleave(
                torch.cat((inputs_x, inputs_u_w, inputs_u_s, inputs_u_s1)),
                3 * args.mu + 1,
            ).to(args.device)

            targets_x = targets_x.to(args.device)

            logits, logits_b = model(inputs)
            # logits = model(logits_feat, classify=True)
            ...
            # logits_b = model(logits_feat, classify1=True)
            ...
            # logger.info(logits_x_b.shape)
            # logger.info(args.adjustment_l2.shape)
            Lx_b = F.cross_entropy(
                logits_x_b + args.adjustment_l2, targets_x, reduction="mean"
            )

            pseudo_label = torch.softmax(
                (logits_u_w.detach() - args.adjustment_l1) / args.T, dim=-1
            )
            # NOTE: adaptive
            #
            pseudo_label_h2 = torch.softmax(
                (logits_u_w.detach() - args.adjustment_l12) / args.T, dim=-1
            )
            # NOTE: not adaptive

            pseudo_label_b = torch.softmax(logits_u_w_b.detach() / args.T, dim=-1)
            # NOTE: balanced branch for balance prediction

            pseudo_label_t = torch.softmax(logits_u_w.detach() / args.T, dim=-1)
            # NOTE: standard branch for bias prediction
            ...
            if epoch > args.est_epoch:
                ...
                if now_mask.sum() > 0:
                    ...
                    KL_con = 0.5 * KL_div(args.py_con.log(), args.u_py) + 0.5 * KL_div(
                        args.u_py.log(), args.py_con
                    )
                    KL_uni = 0.5 * KL_div(args.py_uni.log(), args.u_py) + 0.5 * KL_div(
                        args.u_py.log(), args.py_uni
                    )
                    KL_rev = 0.5 * KL_div(args.py_rev.log(), args.u_py) + 0.5 * KL_div(
                        args.u_py.log(), args.py_rev
                    )
            ...
            Lu = (
                F.cross_entropy(logits_u_s_twice, targets_u_twice, reduction="none")
                * mask_twice_ss_t
            ).mean()

            Lu_b = (
                F.cross_entropy(
                    logits_u_s_b_twice, targets_u_h2_twice, reduction="none"
                )
                * mask_twice_ss_b_h2
            ).mean()
            ...
            semi_acc_b = (
                (targets_u_b[mask_l * mask_b] == u_real[mask_l * mask_b]).float().mean()
            )
            num = (mask_l * mask_b).sum() + 1
            semi_acc.update(semi_acc_b.item(), n=num.item())

            acc = (
                ((logits_x_b + args.adjustment_l2).argmax(dim=1) == targets_x)
                .float()
                .mean()
            )
        ...
        if args.local_rank in [-1, 0]:
            test_loss, test_acc, test_top5_acc, test_acc_b, test_top5_acc_b = test(
...
def test(args, test_loader, model, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            outputs, outputs_b = model(inputs)
            # outputs = model(outputs_feat,classify=True)
            # outputs_b = model(outputs_feat,classify1=True)
